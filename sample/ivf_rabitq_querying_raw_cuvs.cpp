...
int main(int argc, char** argv) {
    ...
    for (size_t c = 0; c < num_cluster; ++c) {
        ...
        for (size_t i = 0; i < n; i += 32) {
            ...
            for (size_t v = 0; v < batch_len; ++v) {
                ...
                // =========================================================================
                // 🌟 核心魔法：FastScan 变态交织内存 100% 精确逆向转置
                // =========================================================================
    ...
    // IVF_RaBitQ_Raw gpu_ivf;
    // gpu_ivf.load_index(index_file);

    // 测试的不同 nprobe 列表
    std::vector<size_t> nprobes = {5,10,20,30,40,60,80,100,150,200}; 
    ...
    for (size_t r = 0; r < test_round; r++) {
        for (size_t l = 0; l < length; ++l) {
            ...
            // 整个 Query 矩阵一次性送入 GPU
            // 注意：query.data() 获取底层连续的一维数组指针
            ...
            // 计算 Recall (与原始逻辑一致，但结果数组变成了一维展平)
            for (size_t i = 0; i < nq; i++) {
